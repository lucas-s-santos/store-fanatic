/**
 * Script: upload-laliga.mjs
 * Faz upload das imagens da La Liga para o Cloudinary e cria os produtos no Supabase.
 *
 * Como rodar:
 *   node scripts/upload-laliga.mjs
 */
...
const BASE_FOLDER   = 'laliga'
const IMAGES_ROOT   = join(__dirname, '..', 'public', 'LaLiga')
const MAP_OUTPUT    = join(__dirname, '..', 'cloudinary-map-laliga.json')
const CONCURRENT    = 5
...
const TEAM_MAP = {
  'Athletic Bilbao':    { id: 'athletic-bilbao',  name: 'Athletic Bilbao' },
  'Atlético de Madrid': { id: 'atletico-madrid',  name: 'Atlético de Madrid' },
  'Barcelona':          { id: 'barcelona',        name: 'Barcelona' },
  'Celta de Vigo':      { id: 'celta-vigo',       name: 'Celta de Vigo' },
  'Espanyol':           { id: 'espanyol',         name: 'Espanyol' },
  'Rayo Vallecano':     { id: 'rayo-vallecano',   name: 'Rayo Vallecano' },
  'Real Betis':         { id: 'real-betis',       name: 'Real Betis' },
  'Real Madrid':        { id: 'real-madrid',      name: 'Real Madrid' },
  'Real Valladolid':    { id: 'real-valladolid',  name: 'Real Valladolid' },
  'Sevilla':            { id: 'sevilla',          name: 'Sevilla' },
  'Villarreal':         { id: 'villarreal',       name: 'Villarreal' },
}

function buildPublicId(filePath) {
  const relative = filePath.replace(IMAGES_ROOT, '').replace(/\\/g, '/')
  const withoutExt = relative.replace(/\.[^/.]+$/, '')
  const clean = withoutExt.replace(/^\//, '').replace(/\s+/g, '-').replace(/[áàãâä]/g, 'a').replace(/[éèêë]/g, 'e').replace(/[íìîï]/g, 'i').replace(/[óòõôö]/g, 'o').replace(/[úùûü]/g, 'u').replace(/[^a-zA-Z0-9/_-]/g, '')
...
async function main() {
  ...
  console.log('🔍 Listando imagens em public/LaLiga...')
  ...
  console.log(`📄 Mapa salvo em: cloudinary-map-laliga.json\n`)
