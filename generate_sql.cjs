...
fs.writeFileSync('temp_constants.cjs', evalCode);
const LEAGUES = require('./temp_constants.cjs');

let sql = '';
LEAGUES.forEach(league => {
  league.teams.forEach(team => {
    sql += `INSERT INTO public.teams (id, league_id, name, logo_url) VALUES ('${team.id}', '${league.id}', '${team.name.replace(/'/g, "''")}', '${team.logo.replace(/'/g, "''")}') ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, logo_url = EXCLUDED.logo_url, league_id = EXCLUDED.league_id;\n`;
  });
});
